...
<div>
  <b-tabs content-class="mt-3" v-model="tabIndex">
    <b-tab title="Union" active></b-tab>
    <b-tab title="Van"></b-tab>
    <b-tab title="Center3"></b-tab>
    <b-tab title="Center4"></b-tab>
  </b-tabs>
</div>

{{-- <p>the value is @{{items[tabIndex].location}}</p> --}}
...
{{--         <div class="col-md-3">
        ...
        <div class="col-md-4">
          <div class="card">
            <div class="card-body">
              <h4 class="mb-3">{{ Str::title(str_replace('_', ' ', $type->name)) }}</h4>
              <div class="d-block my-3">
                ...
                <div class="form-check">
                  <input
                  v-if="!(
                    (items[tabIndex].location === 'union' && !({{ $option->union }})) ||
                    (items[tabIndex].location === 'van' && !({{ $option->van }})) ||
                    (items[tabIndex].location === 'union' && !({{ $option->union }})) ||
                    (items[tabIndex].location === 'union' && !({{ $option->union }}))
                  )"
...
{{-- v-bind:disabled="" --}}
{{--                   v-bind:
                  "['union' ? items[tabIndex].location :  @if(!($option->union)) disabled @endif]" --}}

{{--                   v-if="items[tabIndex].location === 'union'"
                  @if(!($option->union)) disabled @endif

                  v-if="items[tabIndex].location === 'van'"
                  @if(!($option->van)) disabled @endif --}}
    ...
    Upload File:
    <br />
    <input type="file" name="copy_file" />
